...
#返回采样列表
def video_frame_sample(frame_interval, video_length, sample_num):#10，16
    num = []
...
# def video_frame_sample(vid_len, t, sample_num):
#     frame_interval = vid_len / t  # 浮点数，避免取整误差
#     num = []
#     for l in range(t):
#         # 计算当前时间段的起始和结束位置（精确到浮点数）
#         start = l * frame_interval
#         end = (l + 1) * frame_interval
#         # 在当前时间段内均匀采样 sample_num 帧
#         for i in range(sample_num):
#             pos = start + (i / sample_num) * (end - start)
#             n = int(np.round(pos))  # 四舍五入取整
#             n = min(n, vid_len - 1)  # 确保不越界
#             num.append(n)
#     return num
